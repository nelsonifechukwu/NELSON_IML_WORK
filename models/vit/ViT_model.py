...
class PatchEmbed(nn.Module):
    def __init__(self, img_size, patch_size, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size) # convolutional layer to get patches

    def forward(self, x):
        x = self.proj(x)
        x = x.flatten(2) # flatten the last two dimensions
        x = x.transpose(1, 2) # swap the dimensions
...
class VisionTransformer(nn.Module):
    def __init__(self, image_size=384, patch_size=16, in_chans=3, n_classes=2,
                  embed_dim=768, depth=12, n_heads=12, mlp_ratio=4., 
                  qkv_bias=True, p=0., attn_p=0., proj_p=0.):
        super().__init__()

        self.patch_embed = PatchEmbed(image_size, patch_size, in_chans, embed_dim) # patch embedding module
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) # class token
        self.pos_embed = nn.Parameter(torch.zeros(1, self.patch_embed.n_patches + 1, embed_dim)) # positional embedding
